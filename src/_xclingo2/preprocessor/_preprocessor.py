...
class Preprocessor:
    def __init__(self) -> None:
        self._translation = ""
        self._translation_wiv_sim_facts = ""
    ...
    def process_program(self, program: str):
        ...
        """
        self._translation = ""
        # TODO xclingo bug here, annotated #show /0. result in a syntax error
        if '#show /0.' in program:
            # print("___________-program",program)
            program = program.replace('#show /0.','#show.')
        # TODO 
        #print(program)
        parse_string(
            program,
...
class SimPredicatePreprocessor(Preprocessor):
    def __init__(self) -> None:
        super().__init__()
    ...
    def preprocess_rule(self, rule_ast: AST) -> Sequence[AST]:
        return super().preprocess_rule(rule_ast)
