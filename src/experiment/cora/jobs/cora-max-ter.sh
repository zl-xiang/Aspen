...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}.lp -a -m ${EXPGPATH}/maxsol-eqr.lp --main --presimed --ternary --no_show --schema ${SCHEMA}  > ${LOGPATH}/${SCHEMA}-${SUB}-ter-${TIME_STAMP}.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
