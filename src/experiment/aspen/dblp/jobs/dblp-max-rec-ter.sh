...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}.lp -m ${EXPGPATH}/maxsol-eqr.lp  --typed_eval -a --no_show --presimed --ternary --rec-track --schema ${SCHEMA} > ${LOGPATH}/${SCHEMA}-${SUB}-ter-${TIME_STAMP}.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
