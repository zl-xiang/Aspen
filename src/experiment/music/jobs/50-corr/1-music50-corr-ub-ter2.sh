...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}-corr2.lp --main --ub --ter --presimed --typed_eval --schema music --data ${DATA} > ${LOGPATH}/${SCHEMA}${DATA}-${SUB}-ter-${TIME_STAMP}-test2.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
