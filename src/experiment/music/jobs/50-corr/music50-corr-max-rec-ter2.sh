...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}-corr2.lp --typed_eval -a -m ${EXPGPATH}/maxsol-eqr.lp --presimed --ternary --rec-track --data ${DATA} --schema ${SCHEMA} > ${LOGPATH}/${SCHEMA}-${DATA}-${SUB}-ter-${TIME_STAMP}-test2.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
