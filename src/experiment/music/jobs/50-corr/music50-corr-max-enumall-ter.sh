...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}-corr.lp -a -e 0 -m ${EXPGPATH}/maxsol-eqr.lp --main  --ternary --data ${DATA} --typed_eval --presimed --schema ${SCHEMA}  > ${LOGPATH}/${SCHEMA}-${SUB}-ter-${TIME_STAMP}.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
