...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}-corr${VER}.lp  --getsim --ternary --typed_eval --schema ${SCHEMA} --data ${DATA} > ${LOGPATH}/${SCHEMA}-${SUB}-${DATA}-ter-${TIME_STAMP}-test${VER}.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
