...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}.lp --pos-merge rec-2489115-dup-1,rec-2489115-dup-0 --attr release,release   --ternary --presimed --typed_eval --schema music --data ${DATA} > ${LOGPATH}/${SCHEMA}-${SUB}-${DATA}.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
