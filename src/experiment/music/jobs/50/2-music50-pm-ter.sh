...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}.lp --pos-merge all --presimed --typed_eval --ternary --schema music --data ${DATA} > ${LOGPATH}/${SCHEMA}-${SUB}-${DATA}-ter-${TIME_STAMP}.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
