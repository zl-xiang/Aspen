...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${LPATH}/${SCHEMA}-${SPEC}.lp  --main --ub --presimed --ternary --typed_eval --schema ${SCHEMA} --data ${DATA} >  ${LOGPATH}/${SCHEMA}-${SUB}-${DATA}-ter-${TIME_STAMP}.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
