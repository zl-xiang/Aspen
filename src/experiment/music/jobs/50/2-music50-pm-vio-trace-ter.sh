...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -l ${EXPATH}/${SCHEMA}.lp --pos-merge id-748086,id-748010 --attr release,release --trace  --ternary --presimed --typed_eval --schema music --data ${DATA} > ${LOGPATH}/${SCHEMA}-${SUB}-${DATA}_.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
