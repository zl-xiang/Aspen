...
start="$(date +%s)"
#echo $csv_list
python -u mains_explain_.py -c -a -m ${EXPGPATH}/maxsol-eqr.lp -l ${LPATH}/${SCHEMA}-${SPEC}.lp --main --ternary --presimed --typed_eval --schema music --data ${DATA} > ${LOGPATH}/${SCHEMA}-${SUB}-${DATA}-ter-${TIME_STAMP}.log
stop="$(date +%s)"
finish=$(( $stop-$start ))
